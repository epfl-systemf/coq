...
let ssrsettac id ((_, (pat, pty)), (_, occ)) =
  ...
  | Cast(t, DEFAULTcast _, ty) -> t, (sigma, ty)
...
let make_ct t =
  ...
  | _, (_, Some { loc; v = CCast (ct, Some (DEFAULTcast empty_hint), cty)}) ->
    mkt ct, mkt cty, mkt (mkCHole None), loc
  | _, (_, Some ct) ->
    mkt ct, mkt (mkCHole None), mkt (mkCHole None), None
  | _, (t, None) ->
    begin match DAst.get t with
    | GCast (ct, Some (DEFAULTcast empty_hint), cty) ->
...
let wlogtac ist (((clr0, pats),_),_) (gens, ((_, ct))) hint suff ghave =
  ...
  | (a, (b, Some ct)) ->
    ...
    | CCast (_, Some (DEFAULTcast empty_hint), cty) -> a, (b, Some cty)
    | _ -> anomaly "wlog: ssr cast hole deleted by typecheck"
    end
  | (a, (t, None)) ->
    begin match DAst.get t with
    | GCast (_, Some (DEFAULTcast empty_hint), cty) -> a, (cty, None)
    | _ -> anomaly "wlog: ssr cast hole deleted by typecheck"
    end
  in
