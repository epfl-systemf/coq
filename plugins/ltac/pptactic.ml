...
let string_of_genarg_arg (ArgumentType arg) =
  ...
  let pr_zeta_loc _ (sg, x) = Pputils.pr_or_by_notation Libnames.pr_qualid sg ++ pr_opt (pr_or_var int) x
  let pr_zeta_glob env (gr, x) = Termops.pr_global_env env gr ++ pr_opt (pr_or_var int) x
