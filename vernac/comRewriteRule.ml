...
let rec safe_pattern_of_constr_aux ~loc env evd usubst depth state t = Constr.kind t |> function
  ...
  | Case (ci, u, params, ((nas, p), _), _, c, brs) ->
      let mind = Environ.lookup_mind_specif env ci.ci_ind in
      let realdecls, bctxs = Environ.expand_case_contexts mind (ci.ci_ind, u) params nas brs in
      ...
      let do_one_branch i state (nas, br) =
        let br_env = Environ.push_rel_context bctxs.(i) env in
